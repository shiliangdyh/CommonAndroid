...
public class StatusBarUtils {
    ...
    public static void setColor(Activity activity, @ColorInt int color, @IntRange(from = 0, to = 255) int statusBarAlpha) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            //加上这个属性在华为6P弹出系统框的时候出现一个黑色的条条 先去掉
...
//            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
    ...
    public static void setTranslucentForImageView(Activity activity, @IntRange(from = 0, to = 255) int statusBarAlpha,
        ...
        setTransparentForWindow(activity);
        addTranslucentView(activity, statusBarAlpha);
        setViewOffset(activity, needOffsetView);
    }

    /**
     * 向下偏移statusbar高度
     * @param activity
     * @param needOffsetViews
     */
    public static void setViewOffset(Activity activity, View... needOffsetViews) {
    ...
    private static void setViewOffset(Activity activity, View needOffsetView) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
    ...
    public static boolean setLightMode(Activity activity) {
        boolean miuiSuccess = setMIUIStatusBarDarkIcon(activity, true);
        boolean meizuSuccess = setMeizuStatusBarDarkIcon(activity, true);
        ...
        return miuiSuccess || meizuSuccess || Build.VERSION.SDK_INT >= Build.VERSION_CODES.M || Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT;
    }

    @TargetApi(Build.VERSION_CODES.M)
    public static boolean setDarkMode(Activity activity) {
        boolean miuiSuccess = setMIUIStatusBarDarkIcon(activity, false);
        boolean meizuSuccess = setMeizuStatusBarDarkIcon(activity, false);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
...
//            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
    ...
    public static void setFitsSystemWindows(View view, boolean fitsSystemWindows) {
        if (view == null){
            return;
        }
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
